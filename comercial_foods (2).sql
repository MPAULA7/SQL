...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 07-03-2024 a las 16:01:59
...
CREATE TABLE `empleado` (
  ...
  `telefono` varchar(25) NOT NULL,
  `fecha_ingreso` date DEFAULT curdate(),
  `estado_civil` enum('soltero','casado','divorciado','union lire','viudo') DEFAULT NULL,
  `cargo_cod` int(11) NOT NULL,
  `nivel_estudios` enum('primaria','bachillerato','tecnio','tecnlogo','profesional','otro') DEFAULT NULL,
  `eps` enum('sanitas','sura','capital salud','nueva\r\n    eps','compensar','famisanar','aliansalud') DEFAULT NULL,
  `pensiones` enum('colfondos','proteccion','porvenir','skandia') DEFAULT NULL,
  `cesantias` enum('fna','portvenir','colfonods','proteccion') DEFAULT NULL,
  `bacno` enum('BBVA','DAVIVIENDA','bancolombia','caja social','popular','las villas') DEFAULT NULL,
  `activo` varchar(4) NOT NULL DEFAULT 'A'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
--
ALTER TABLE `producto`
  ADD CONSTRAINT `producto_ibfk_1` FOREIGN KEY (`categor_cod`) REFERENCES `categoria` (`cod_categoria`),
